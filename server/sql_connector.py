    ...
    def DBconnection(self):
        try:
            ...
            try:
                ...
                self.db_cursor.execute('CREATE TABLE IF NOT EXISTS AccountTable (ID INT, Year INT, Month INT, name VARCHAR(20) NOT NULL, program INT, foodExpense INT, bookExpense INT, Total INT, isPaid BOOL DEFAULT FALSE, 備註 VARCHAR(200))')
                self.db_cursor.execute('CREATE TABLE IF NOT EXISTS FoodExpenseDetail (ID INT, Year INT, Month INT, Day INT, name VARCHAR(20) NOT NULL, price INT DEFAULT 0, isCounted BOOL DEFAULT FALSE, 備註 VARCHAR(200))')
